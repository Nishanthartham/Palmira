...
from .check_condition_lattice_bbox.utils import check_condition_f_bbox
from .mean_feature.mean_feature import get_grid_mean_feature
from .variance_function_atom.line_distance_func_topk.utils import (
    variance_f_in_one_atom_topk as line_variance_topk,
)
...
class LatticeVariance(nn.Module):
    def __init__(
        self, output_row, output_column, device, sigma=0.001, add_seg=False, mask_coef=0.3
    ):
    ...
    def forward(
        ...
        gather_input = grid_pos.unsqueeze(2).expand(
            n_batch, grid_pos.shape[1], 3, 2
        )  # [None, 196, 3, 2]
        gather_index = (
            base_triangle2point.unsqueeze(-1)
            .expand(n_batch, base_triangle2point.shape[1], 3, 2)
            .long()
        )  # [None, 338, 3, 2]
        lattice = torch.gather(input=gather_input, dim=1,
                               index=gather_index)  # [None, 338, 3, 2]
        ...
        if self.add_seg:
            ...
            lab_img_fea = torch.cat(
                (lab_img_fea, semantic_mask * self.mask_coef), dim=-1
            )  # [None, 28, 28, 5]

        variance_bxp, reconstruct_fea, condition = self.variance(
            lattice, lab_img_fea, inference=inference, img_pos=img_pos, grid_size=grid_size
        )
        ...
        area_variance = self.area_variance(
            lattice, base_area_mask, area_normalize)
    ...
    def reconstruct(self, reconstruct_fea, img_fea):
        ...
        reconstruct_loss = torch.abs(
            reconstruct_fea - img_fea).reshape(batch_size, -1).mean(dim=-1)
    ...
    def variance(self, lattice, img_fea, inference, img_pos=None, grid_size=None):
        ...
        if img_pos is None:
            img_pos = self.output_pos.unsqueeze(0).expand(
                n_batch, self.output_row, self.output_column, 2
            )  # [None, 28, 28, 2]

        img_pos = img_pos.view(n_batch, -1, 2)
        img_pos = img_pos.contiguous()  # [None, 784, 2]

        condition = check_condition_f_bbox(lattice, img_pos)  # [None, 196, 1]

        if inference:
            return None, None, condition

        # Expect img_fea to have shape Bxpxd and condition to have shape Bxp
        img_fea = img_fea.reshape(
            n_batch, -1, img_fea.shape[-1])  # [None, 784, 5]
        grid_fea, _ = get_grid_mean_feature(
            img_fea, condition.squeeze(-1), grid_num)
        ...
        variance_bxp, reconstruct_img = line_variance_topk(
            img_fea, grid_fea, lattice, img_pos, new_sigma
        )
    ...
    def area_variance(self, lattice, base_area_mask, area_normalize=(20, 20)):
        tmp_lattice = lattice * torch.tensor([area_normalize[1], area_normalize[0]]).float().to(
            lattice.device
        ).reshape(1, 1, 1, 2)
        A = tmp_lattice[:, :, 0, :]
        B = tmp_lattice[:, :, 1, :]
