...
class MeanFeatureGather(torch.autograd.Function):
    ...
    def forward(ctx, feature_map, condition, grid_num):
        ...
        mean_feature = torch.zeros(
            batch_size, grid_num, feature_channel_num, device=feature_map.device
        )
        ...
        if feature_map.shape[1] == 100000000:
            for i_batch in range(feature_map.shape[0]):
                ...
                mean_feature_func.forward_cuda(
                    mean_feature[i_batch].unsqueeze(0),
                    grid_size[i_batch].unsqueeze(0),
                    tmp_feature.float(),
                    condition[i_batch].unsqueeze(0).float(),
                )

        else:
            feature_map = feature_map.contiguous()
            mean_feature_func.forward_cuda(
                mean_feature, grid_size, feature_map.float(), condition.float()
            )
    ...
    def backward(ctx, grad_mean_feature, grad_grid_size):
        ...
        if filtered_condition.shape[-1] == 100000000:
            ...
            for i_batch in range(filtered_condition.shape[0]):
                ...
                feature_map_weights = torch.gather(
                    input=grid_size[i_batch].unsqueeze(0), dim=1, index=local_condition
                ).detach()  # for each pixel, how manny pixels in total its corresponding grid has
                tmp_shape = local_condition.shape
                feature_channel_num = grad_mean_feature[i_batch].shape[-1]
                tmp_condition = local_condition.unsqueeze(-1).expand(
                    tmp_shape[0], tmp_shape[1], feature_channel_num
                )

                feature_map_grad = torch.gather(
                    input=grad_mean_feature[i_batch].unsqueeze(0), dim=1, index=tmp_condition
                ).detach()  # for each pixel, the grad vector of one position of its corresponding grid
                feature_map_grad = feature_map_grad / feature_map_weights.reshape(
                    tmp_shape[0], tmp_shape[1], 1
                )  # divided by the total number of pixels one grid has
            ...
            feature_map_weights = torch.gather(
                input=grid_size, dim=1, index=filtered_condition
            ).detach()  # for each pixel, how manny pixels in total its corresponding grid has
            tmp_shape = filtered_condition.shape
            feature_channel_num = grad_mean_feature.shape[-1]
            tmp_condition = filtered_condition.unsqueeze(-1).expand(
                tmp_shape[0], tmp_shape[1], feature_channel_num
            )

            feature_map_grad = torch.gather(
                input=grad_mean_feature, dim=1, index=tmp_condition
            ).detach()  # for each pixel, the grad vector of one position of its corresponding grid
            feature_map_grad = feature_map_grad / feature_map_weights.reshape(
                tmp_shape[0], tmp_shape[1], 1
            )  # divided by the total number of pixels one grid has
