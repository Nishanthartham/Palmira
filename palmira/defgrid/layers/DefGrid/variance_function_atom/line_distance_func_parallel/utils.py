...
class VarianceFunc(Function):
    ...
    @staticmethod
    def forward(
        ctx, img_fea_bxnxd, grid_fea_bxkxd, grid_bxkx3x2, img_pos_bxnx2, sigma, condition, eps=1e-5
    ):
        # import ipdb
        # ipdb.set_trace()
        ...
        buffer_1xnxk = torch.zeros(
            1,
            int(n_pixel / n_img_split) + n_img_split - 1,
            n_grid,
            device=grid_bxkx3x2.device,
            dtype=torch.float,
        )
        buffer_1xn = torch.zeros(
            1,
            int(n_pixel / n_img_split) + n_img_split - 1,
            device=grid_bxkx3x2.device,
            dtype=torch.float,
        )
        buffer_1xnx4 = torch.zeros(
            1,
            int(n_pixel / n_img_split) + n_img_split - 1,
            split_size,
            device=grid_bxkx3x2.device,
