...
line_variance_topk = load(
    name='line_variance_topk',
    sources=[
        os.path.join(
            base_path,
...
class VarianceFunc(Function):
    ...
    @staticmethod
    def forward(
        ctx, img_fea_bxnxd, grid_fea_bxkxd, grid_bxkx3x2, img_pos_bxnx2, sigma, eps=1e-5, topk=50
    ):
        # import ipdb
        # ipdb.set_trace()
        ...
        reconstruct_img = torch.zeros(
            n_batch,
            n_pixel,
            grid_fea_bxkxd.shape[-1],
        ...
        buffer_bxnxk = torch.zeros(
            n_batch, n_pixel, topk, device=grid_bxkx3x2.device, dtype=torch.float
        )
        ...
        ctx.save_for_backward(
            img_fea_bxnxd, grid_fea_bxkxd, grid_bxkx3x2, img_pos_bxnx2, top_k_grid, sigma
        )
    ...
    def backward(ctx, dldvariance_bxn, dldreconstruct_img_bxnxd):
        ...
        dldgrid_bxkx3x2 = torch.zeros(
            n_batch, n_grid, 3, 2, device=grid_bxkx3x2.device, dtype=torch.float
        )
        buffer_bxnxk = torch.zeros(
            n_batch, n_pixel, topk, device=grid_bxkx3x2.device, dtype=torch.float
        )
        line_variance_topk.backward(
            dldvariance_bxn,
            img_fea_bxnxd,
            grid_fea_bxkxd,
            grid_bxkx3x2,
